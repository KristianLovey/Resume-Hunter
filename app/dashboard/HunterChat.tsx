...
const SUGGESTIONS = [
  "How can I improve the description of my last job?",
  "What do I write in a cover letter with no experience?",
  "Which skills am I missing for a junior developer role?",
    ...
    >
      <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--rh-border-soft)", display: "flex", alignItems: "center", gap: 11 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 14.5, fontWeight: 800, color: "var(--rh-text)" }}>Ask Hunter</div>
          <div style={{ fontSize: 12, color: "var(--rh-text-3)" }}>Resumes, cover letters, job ads and interviews</div>
        </div>
      </div>
      ...
      <div ref={scrollRef} style={{ padding: "18px 20px", minHeight: 220, maxHeight: 420, overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {isSending && (
          <div style={{ display: "flex", gap: 5, alignItems: "center", padding: "6px 2px" }}>
            ...
            <span style={{ fontSize: 12.5, color: "var(--rh-text-3)", marginLeft: 6 }}>Hunter is thinking…</span>
          </div>
        )}
      ...
      <div style={{ padding: "14px 16px", borderTop: "1px solid var(--rh-border-soft)", background: "var(--rh-surface-2)" }}>
        <div className="rh-fieldwrap" style={{ display: "flex", gap: 9, alignItems: "flex-end", border: "1px solid var(--rh-border)", borderRadius: 13, background: "var(--rh-surface)", padding: 7 }}>
          <textarea
            ...
            rows={2}
            maxLength={2000}
            placeholder="e.g. How should I describe my internship?"
        ...
        <div style={{ marginTop: 9, fontSize: 11.5, color: "var(--rh-text-3)", lineHeight: 1.5 }}>
          Hunter only answers questions about resumes, cover letters and job hunting. Always check its answers before sending anything to an employer.
        </div>
      </div>
    </div>
  );
